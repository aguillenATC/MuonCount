...
svm_model = manager.fit_svm_regression()


if not os.path.isdir(global_dirs.results_path):
    os.mkdir(global_dirs.results_path)
if not os.path.isdir(global_dirs.svm_path):
    os.mkdir(global_dirs.svm_path)
if not os.path.isdir(global_dirs.svm_path+"scalers/"):
    os.mkdir(global_dirs.svm_path+"scalers/")
if not os.path.isdir(global_dirs.svm_path+"model/"):
    os.mkdir(global_dirs.svm_path+"model/")
if not os.path.isdir(global_dirs.svm_path + "results/"):
    os.mkdir(global_dirs.svm_path + "results/")

if(isinstance(scalers,tuple)):
    joblib.dump(scalers[0], global_dirs.svm_path + "scalers/scaler_X.h5")
    joblib.dump(scalers[1], global_dirs.svm_path + "scalers/scaler_y.h5")
else:
    joblib.dump(scalers, global_dirs.svm_path + "scalers/scaler_X.h5")
joblib.dump(svm_model, global_dirs.svm_path + "/model/model.h5")
...
for name,ds in test.items():
    ...
    ax[1].set_title("SVM Regression (SVR)\nQGSJET-II (test)\nReal vs. Predicted - {}".format(name))
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.svm_path + "results/qgsjet-results-{}.png".format(name), dpi=70)
...
for name,ds in test.items():
    ...
    #plt.show()
    plt.savefig(global_dirs.svm_path + "results/epos-results-{}.png".format(name), dpi=70)


with open(global_dirs.svm_path+"best_params.txt","w") as f:
    f.write("Best params: {}\n".format(results["best_params"]))


import pickle
best_params=results["best_params"]
f = open(global_dirs.svm_path+'best_params.pkl', 'wb')
...
for name,ds in test.items():
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.svm_path + "results/qgsjet-results-WHOLE-{}.png".format(name), dpi=70)