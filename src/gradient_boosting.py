...
#max_depth_tuning=np.arange(1,15)
#best_depth=1
#best_r2=0
#history_r2=[]
#Gradient boosting
#for md in max_depth_tuning:
#    manager.fit_boost_regression(max_depth=md)
#    r2=manager.predict_boost_regression(validation, tup)["r2"]
...
gb_model = manager.fit_boost_regression()

if not os.path.isdir(global_dirs.results_path):
    os.mkdir(global_dirs.results_path)
if not os.path.isdir(global_dirs.gradient_boosting_path):
    os.mkdir(global_dirs.gradient_boosting_path)
if not os.path.isdir(global_dirs.gradient_boosting_path+"scalers/"):
    os.mkdir(global_dirs.gradient_boosting_path+"scalers/")
if not os.path.isdir(global_dirs.gradient_boosting_path+"model/"):
    os.mkdir(global_dirs.gradient_boosting_path+"model/")
if not os.path.isdir(global_dirs.gradient_boosting_path + "results/"):
    os.mkdir(global_dirs.gradient_boosting_path + "results/")

if(isinstance(scalers,tuple)):
    joblib.dump(scalers[0], global_dirs.gradient_boosting_path + "scalers/scaler_X.h5")
    joblib.dump(scalers[1], global_dirs.gradient_boosting_path + "scalers/scaler_y.h5")
else:
    joblib.dump(scalers, global_dirs.gradient_boosting_path + "scalers/scaler_X.h5")
joblib.dump(gb_model, global_dirs.gradient_boosting_path + "/model/model.h5")
...
#plt.savefig(global_dirs.gradient_boosting_path + "results/max_depth_tuning.png",dpi=70)
##plt.show()
#plt.close()
...
for name,ds in test.items():
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.gradient_boosting_path + "results/qgsjet-results-{}.png".format(name), dpi=70)
...
for name,ds in test.items():
    ...
    #plt.show()
    plt.savefig(global_dirs.gradient_boosting_path + "results/epos-results-{}.png".format(name), dpi=70)


with open(global_dirs.gradient_boosting_path+"best_params.txt","w") as f:
    f.write("Best params: {}\n".format(results["best_params"]))

import pickle
best_params=results["best_params"]
f = open(global_dirs.gradient_boosting_path+'best_params.pkl', 'wb')
...
for name,ds in test.items():
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.gradient_boosting_path + "results/qgsjet-results-WHOLE-{}.png".format(name), dpi=70)