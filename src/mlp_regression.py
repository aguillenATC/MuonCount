...
if not os.path.isdir(global_dirs.mlp_path+"model/"):
    os.mkdir(global_dirs.mlp_path+"model/")
if not os.path.isdir(global_dirs.mlp_path + "results/"):
    os.mkdir(global_dirs.mlp_path + "results/")

if(isinstance(scalers,tuple)):
    joblib.dump(scalers[0], global_dirs.mlp_path + "scalers/scaler_X.h5")
    joblib.dump(scalers[1], global_dirs.mlp_path + "scalers/scaler_y.h5")
else:
    joblib.dump(scalers, global_dirs.mlp_path + "scalers/scaler_X.h5")
joblib.dump(mlp_model, global_dirs.mlp_path + "/model/model.h5")
...
for name,ds in test.items():
    results=manager.predict_mlp_regression(ds,tup)
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.mlp_path + "results/qgsjet-results-{}.png".format(name), dpi=70)
...
for name,ds in test.items():
    ...
    f#fit = np.polyfit(results["y_true"], results["y_predicted"], 1)
    #fit_fn = np.poly1d(fit)
    ax[1].scatter(results["y_true"], results["y_predicted"], c=colors[name], marker="o")
    #ax[1].plot(results["y_true"], fit_fn(results["y_true"]), '--b', lw=2)
    top_axis = int(max(max(results["y_true"]), max(results["y_predicted"]))) + 1
    ...
    #plt.show()
    plt.savefig(global_dirs.mlp_path + "results/epos-results-{}.png".format(name), dpi=70)


with open(global_dirs.mlp_path+"best_params.txt","w") as f:
    f.write("Best params: {}\n".format(results["best_params"]))


import pickle
best_params=results["best_params"]
f = open(global_dirs.mlp_path+'best_params.pkl', 'wb')
...
for name,ds in test.items():
    ...
    ax[1].grid(alpha=0.5)
    # plt.show()
    plt.savefig(global_dirs.mlp_path + "results/qgsjet-results-WHOLE-{}.png".format(name), dpi=70)