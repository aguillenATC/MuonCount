...
#    manager.fit_dt_regression(max_depth=md)
#    r2=manager.predict_dt_regression(validation, tup)["r2"]
...
#dt_model = manager.fit_dt_regression(max_depth=best_depth)

dt_model = manager.fit_dt_regression()

if not os.path.isdir(global_dirs.results_path):
    os.mkdir(global_dirs.results_path)
if not os.path.isdir(global_dirs.decision_tree_path):
    os.mkdir(global_dirs.decision_tree_path)
if not os.path.isdir(global_dirs.decision_tree_path+"scalers/"):
    os.mkdir(global_dirs.decision_tree_path+"scalers/")
if not os.path.isdir(global_dirs.decision_tree_path+"model/"):
    os.mkdir(global_dirs.decision_tree_path+"model/")
if not os.path.isdir(global_dirs.decision_tree_path + "results/"):
    os.mkdir(global_dirs.decision_tree_path + "results/")

if(isinstance(scalers,tuple)):
    joblib.dump(scalers[0], global_dirs.decision_tree_path + "scalers/scaler_X.h5")
    joblib.dump(scalers[1], global_dirs.decision_tree_path + "scalers/scaler_y.h5")
else:
    joblib.dump(scalers, global_dirs.decision_tree_path + "scalers/scaler_X.h5")
joblib.dump(dt_model, global_dirs.decision_tree_path + "/model/model.h5")
...
#plt.savefig(global_dirs.decision_tree_path + "results/max_depth_tuning.png",dpi=70)
##plt.show()
#plt.close()
...
for name,ds in test.items():
    results=manager.predict_dt_regression(ds,tup)
    ...
    rho_patch = mpatches.Patch(color='white', label=r"Pearson's $\rho$: {}".format(round(results["pearson_rho"], dec_round)))
    pval_patch = mpatches.Patch(color='white', label=r"Pearson's $\rho$ p-val: {}".format(round(results["pearson_pval"], dec_round)))
    ax[1].legend(handles=[r2_patch, mae_patch, mse_patch,rho_patch,pval_patch], loc="upper left")
    ax[1].grid(alpha=0.5)

    # plt.show()
    plt.savefig(global_dirs.decision_tree_path + "results/qgsjet-results-{}.png".format(name), dpi=70)
...
for name,ds in test.items():
    ...
    #plt.show()
    plt.savefig(global_dirs.decision_tree_path + "results/epos-results-{}.png".format(name), dpi=70)



with open(global_dirs.decision_tree_path+"best_params.txt","w") as f:
    f.write("Best params: {}\n".format(results["best_params"]))

import pickle
best_params=results["best_params"]
f = open(global_dirs.decision_tree_path+'best_params.pkl', 'wb')
...
for name,ds in test.items():
    ...
    # plt.show()
    plt.savefig(global_dirs.decision_tree_path + "results/qgsjet-results-WHOLE-{}.png".format(name), dpi=70)